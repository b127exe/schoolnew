...
if (isset($_GET['searchStu'])) {
  ...
  $query = "SELECT * FROM student AS s INNER JOIN parent AS p ON s.pid = p.pid INNER JOIN class AS c ON s.cid = c.cid INNER JOIN section AS sec ON c.section = sec.sec_id INNER JOIN subject AS su ON s.subid = su.subid INNER JOIN fees AS f ON su.fid = f.fid INNER JOIN teacher AS t ON su.tid = t.tid WHERE $list LIKE '%$search%'";
} else {
  $query = "SELECT * FROM student AS s INNER JOIN parent AS p ON s.pid = p.pid INNER JOIN class AS c ON s.cid = c.cid INNER JOIN section AS sec ON c.section = sec.sec_id INNER JOIN subject AS su ON s.subid = su.subid INNER JOIN fees AS f ON su.fid = f.fid INNER JOIN teacher AS t ON su.tid = t.tid";
}
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-lg-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="display-4 font-weight-bold text-dark text-center">Fees </h4>
                        ...
                        if (mysqli_num_rows($result) > 0) {
                          while ($row = mysqli_fetch_assoc($result)) {
                            echo "<tr>
                                      ...
                                      <td><a href='feesRecord.php?sid=$row[sid]&fid=$row[fid]' class='btn btn-secondary btn-rounded btn-sm'>View</a><td>
                                     </tr>";
                          }
                        } else {
